...
> **Superseded by [netbox-client-refactor-plan-v2.md](./netbox-client-refactor-plan-v2.md).**
>
> The v2 plan refines the architecture with `internal/restapi` (instead of `internal/transport`), introduces `internal/netbox/models` to eliminate circular dependencies, and standardizes on singular API struct names.
